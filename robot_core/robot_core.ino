...
//============================ LOOP =============================
  void loop()
  {
    ...
    tempinc.temperature = sensors.getTempCByIndex(0);
    // allow modules to process data for this loop iteration.
    
    // === Air Quality ===================
       // For LPG Gas (in PPM)
       airQuality.gas_lpg = MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_LPG);  
   //  For Gas CO (in ppm)  
       airQuality.gas_co = MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_CO);
     // for Gas Smoke (in ppm) 
       airQuality.gas_smoke = MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_SMOKE);
